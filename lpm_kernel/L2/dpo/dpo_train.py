...
def training_data_processor(args, SYS = "You are a helpful assistant.\n\n"):
    with open(args.training_data_path, "r", encoding="utf-8") as f:
        data = json.load(f)
    # Support two prompt formats:
    # 1) Legacy: {"prompt": {"system": "...", "user": "..."}, "chosen": "...", "rejected": "..."}
    # 2) New:    {"prompt": [{"role":"system","content":"..."}, ...], "chosen":"...", "rejected":"..."}
    training_data = {
        "prompt": [
            (
                data_point["prompt"]
                if isinstance(data_point.get("prompt"), list)
                else [
                    {"role": "system", "content": data_point["prompt"].get("system", "")},
                    {"role": "user", "content": data_point["prompt"].get("user", "")},
                ]
            )
